...
var metrics = struct {
	...
	userAppCredentialCountGauge prometheus.Gauge
	...
	userAppCredentialCountGauge: promauto.NewGauge(prometheus.GaugeOpts{Name: "todo_db_user_app_credentials", Help: "Total number of user app credentials"}),
...
func (db *DB) updateStats(ctx context.Context) error {
	...
	if err = tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM user_app_credentials`).Scan(&n); err != nil {
		return fmt.Errorf("user_app_credentials count: %v", err)
	}
	metrics.userAppCredentialCountGauge.Set(float64(n))
